...
$.widget("nm.account", {
  ...
  _refresh: function(){
    ...
    if(o.client.getCurrentUser().isAnonymous()){
      this.$profileMenu.detach();
      this.$loginMenu.appendTo(this.element);
      this.$nickname.text('Please sign-in');
    } else {
      this.$loginMenu.detach();
      this.$profileMenu.appendTo(this.element);
      this.$nickname.text(o.client.getCurrentUser().id);
    }
  },
